...
public class BookingUtil {
    ...
    public static boolean updateBooking(Booking booking) {
        ...
        String query = "UPDATE booking SET vehicle_id = ?, driver_id = ?, city_distance_id = ?, "
                + "booking_date = ?, status = ?, fare = ?, payment_status = ? WHERE booking_id = ?";
        ...
        try (Connection conn = DBConn.getInstance().getConnection();
            ...
            pstmt.setDouble(6, booking.getFare());
            pstmt.setString(7, booking.getPayment_status());
